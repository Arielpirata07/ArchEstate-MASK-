...
class SmsSimulatedVerifier(OTPChannel):
    ...
    def send(self, phone_e164, code, ttl_minutes=10):
        try:
            ...
            if self._audit:
                self._audit("OTP enviado por SMS", f"phone_hash={phone_e164[-6:]} channel=sms ttl={ttl_minutes}m")
...
class WhatsAppSimulatedVerifier(OTPChannel):
    ...
    def __init__(self, audit_fn=None, base_url: str = "https://wa.me"):
        self._audit = audit_fn
        ...
        try:
            ...
            if self._audit:
                self._audit("OTP enviado por WhatsApp",
                            f"phone_hash={phone_e164[-6:]} channel=whatsapp ttl={ttl_minutes}m")
            return SendResult(ok=True, channel=self.name,
...
def get_default_router() -> VerifierRouter:
    ...
    if _default_router is None:
        ...
        _default_router = VerifierRouter(
            ...
            whatsapp_verifier=WhatsAppSimulatedVerifier(audit_fn=log_action),
